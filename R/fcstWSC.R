...
fcstWSC <- function(sesRain, var.dF, rainTerc, variable) {
  if (variable == "onset") {
    ...
    rr.tercCUT <- quantile(var.dF$sRain,
      probs = c(1 / 3, 2 / 3),
      na.rm = TRUE
    )
    ...
    resa.t1 <- sample(
      x = var.dF.t1$onset.Value[!is.na(var.dF.t1$onset.Value)],
      size = idx.t1,
      replace = TRUE
    )
    ...
    resa.t2 <- sample(
      x = var.dF.t2$onset.Value[!is.na(var.dF.t2$onset.Value)],
      size = idx.t2,
      replace = TRUE
    )
    ...
    resa.t3 <- sample(
      x = var.dF.t3$onset.Value[!is.na(var.dF.t3$onset.Value)],
      size = idx.t3,
      replace = TRUE
    )

    var.dF.Ens <- data.frame(
      Members = seq_along(c(resa.t1, resa.t2, resa.t3)),
    ...
    idx.t1 <- round(rainTerc$T1 * nrow(var.dF))
    resa.t1 <- sample(
      x = var.dF.t1$cessation.Value[!is.na(var.dF.t1$cessation.Value)],
      size = idx.t1,
      replace = TRUE
    )
    ...
    idx.t2 <- round(rainTerc$T2 * nrow(var.dF))
    resa.t2 <- sample(
      x = var.dF.t2$cessation.Value[!is.na(var.dF.t2$cessation.Value)],
      size = idx.t2,
      replace = TRUE
    )
    ...
    idx.t3 <- round(rainTerc$T3 * nrow(var.dF))
    resa.t3 <- sample(
      x = var.dF.t3$cessation.Value[!is.na(var.dF.t3$cessation.Value)],
      size = idx.t3,
      replace = TRUE
    )
